...
Парсер лирики AntiHype: [python_src/parse_to_get_text.py](./python_src/parse_to_get_text.py)

Результат: [images_text_src/antihype_train_lyrics.txt](./images_text_src/antihype_train_lyrics.txt)
...
Важно: `LEN()` в YQL считает **байты**, а не символы. Для русских букв в UTF-8 одна буква = 2 байта. Поэтому `LEN(word) > 5` даёт странный результат на кириллице, а `Unicode::Length` — корректный.
...
**Скриншот таблицы:** добавить в `images_text_src/` (зайти в UI и сфоткать `//home/ilyakubashin/mapreduce/word_index`)
